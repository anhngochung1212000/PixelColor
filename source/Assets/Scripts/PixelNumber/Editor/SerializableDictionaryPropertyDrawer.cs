...
public class SerializableDictionaryPropertyDrawer : PropertyDrawer
{
	...
	{
		float labelWidth = 200;// EditorGUIUtility.labelWidth;
	...
	static float DrawKeyValueLineExpand(SerializedProperty keyProperty, SerializedProperty valueProperty, Rect linePosition)
	{
        ...
        float valuePropertyHeight = EditorGUI.GetPropertyHeight(valueProperty);
		SerializedProperty lst = valueProperty.FindPropertyRelative("data");
		if (lst != null)
        {
            linePosition = new Rect(linePosition.x+20f, linePosition.y + EditorGUIUtility.singleLineHeight, linePosition.width, linePosition.height);
			valuePropertyHeight += EditorGUIUtility.singleLineHeight;
	...
	public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
	{
		...
		if (property.isExpanded)
		{
			...
			foreach(var entry in EnumerateEntries(keysProperty, valuesProperty))
			{
				...
				if (lst != null)
				{
					valuePropertyHeight += EditorGUIUtility.singleLineHeight*1.1f;
				}
				float lineHeight = Mathf.Max(keyPropertyHeight, valuePropertyHeight);
