...
public class UIPixelColor : MonoBehaviour
{
    ...
    void OnLoadUIColorItem(List<Color> colors)
    {
        ...
        if(CameraController2D.Instance != null && CameraController2D.Instance.gameObject.activeSelf)
        {
            CameraController2D.Instance.hasMainModel = true;
            CameraController2D.Instance.OnZoomComplete();
        }
    ...
    {
        if (CameraController2D.Instance != null && CameraController2D.Instance.gameObject.activeSelf)
            CameraController2D.Instance.ZoomInCamera();
    ...
    {
        if (CameraController2D.Instance != null && CameraController2D.Instance.gameObject.activeSelf)
            CameraController2D.Instance.ZoomOutCamera();
        if (CameraController3D.Instance != null && CameraController3D.Instance.gameObject.activeSelf)
    ...
    void CheckAllColorPainted()
    {
        ...
        if(isFinish)
        {
            ...
            if (CameraController2D.Instance != null && CameraController2D.Instance.gameObject.activeSelf)
                CameraController2D.Instance.BackToRootPoint();
            if (CameraController3D.Instance != null && CameraController3D.Instance.gameObject.activeSelf)
