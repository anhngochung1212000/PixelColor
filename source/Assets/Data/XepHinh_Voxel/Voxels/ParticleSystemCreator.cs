...
namespace Voxels
{
    ...
    [AddComponentMenu("Voxels/Particle System Creator"), RequireComponent(typeof(Rasterizer))]
    public class ParticleSystemCreator : Processor
    {
        // Template container including a particle system
        public GameObject template;
        // Sizing factor for a particle
        public float sizeFactor = 1;
        // Name of the material property to use as input for the particle color
        ...
        // Identifier of the input material color
        int colorInputIdentifier;
        ...
        public override float Build(Storage voxels, Bounds bounds, Informer informer, object parameter)
        {
            ...
            if (voxels != null)
            {
                ...
                if (width * height * depth * sides > 0)
                {
                    if (particles == null)
                    {
                        ...
                        // Retrieve identifier for input color property
                        colorInputIdentifier = string.IsNullOrWhiteSpace(colorInput) ? 0 : Shader.PropertyToID(colorInput);
                    }
                            ...
                            //for (y = currentHeight; y < currentHeight + 1; ++y)
                            {
                                for (x = 0; x < width; ++x)
                                {
                                    ...
                                    if (material != null)
                                    {
                                        ...
                                        // Get input color
                                        Color color;
                                        if (colorInputIdentifier != 0)
                                        {
                                            color = material.GetColor(colorInputIdentifier);
                                        }
                                        else
                                        {
                                        ...
                                        particles[particlesCount].startColor = color;
            ...
            if (particles != null && particlesCount > 0)
            {
                ...
                if (particlesContainer != null)
                {
                    ...
                    informer?.Invoke(new UnityEngine.Object[] { particlesContainer }, parameter);
                }
            }
